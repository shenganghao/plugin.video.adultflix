...
from packlib import client, kodi, log_utils, url_dispatcher
import os
import pyxbmct
import re
import shutil
...
url_dispatcher = url_dispatcher.URL_Dispatcher()
...
def buildDir(items, content='dirs', cm=None, search=False, stopend=False, isVideo=False, isDownloadable=False, cache=True, chaturbate=False, pictures=False):
    ...
    for i in items:
        ...
        try:
            ...
            u = '%s?url=%s&mode=%s&name=%s&iconimage=%s&fanart=%s' % (sysaddon, quote_plus(url), str(i['mode']), name, quote_plus(thumb), quote_plus(fanart))
            ...
            if chaturbate:
                ...
                chat_compiled = '%s?url=%s&mode=%s&name=%s&iconimage=%s&chat=%s&chatmode=%s&folder=%s' % (sysaddon, quote_plus(check_url), str('101'), name, quote_plus(thumb), chat, str(i['mode']), str(_folder))
            ...
            favorite = '%s?url=%s&mode=%s&name=%s&iconimage=%s&fav=%s&favmode=%s&folder=%s' % (sysaddon, quote_plus(url), str('100'), name, quote_plus(thumb), fav, str(i['mode']), str(_folder))
            ...
            if isDownloadable:
                dwnld = '%s?url=%s&mode=%s&name=%s&iconimage=%s' % (sysaddon, quote_plus(url), str('26'), name, quote_plus(thumb))
                cm.append(('Download Video', 'xbmc.RunPlugin(' + dwnld + ')'))
            if isDownloaded:
                rmdwnld = '%s?url=%s&mode=%s&name=%s' % (sysaddon, quote_plus(url), str('28'), name)
            ...
            if isDownloadable:
                ...
                view_type = 'list'
            view_compile = '%s?mode=%s&name=%s' % (sysaddon, str('44'), view_type)
            ...
            log_utils.log('Error adding item %s in BuildDir function ( %s %s ):: Error: %s' % (name, url, thumb, str(e)), xbmc.LOGERROR)
...
def depVersions():
    ...
    try:
        xml1 = client.request('https://raw.githubusercontent.com/tvaddonsco/tva-adult-repo/master/addons.xml')
        xml2 = client.request('https://raw.githubusercontent.com/tvaddonsco/tva-adult-repo/master/addons.xml')
        lst = [('plugin.video.adultflix'), ('script.adultflix.scrapers'), ('script.adultflix.artwork'), ('script.module.librarypack'), ('script.module.resolveurl'), ('script.module.resolveurl.xxx')]
        ...
        for i in lst:
            ...
            c += [
                (
                    kodi.giveColor(addon_name, 'white', True) + kodi.giveColor('| Installed: ', 'white') + kodi.giveColor(addon_version, ver_color, True) + ' - ' + kodi.giveColor('Available: ', 'white', True) + kodi.giveColor(cur_ver, 'pink', True),
                    icon, kodi.addonfanart, addon_description
                )
            ]
        ...
        for e in c:
            dirlst.append(
                {
                    'name': kodi.giveColor(e[0], 'white'), 'url': 'None', 'mode': 999, 'icon': e[1], 'fanart': e[2],
                    'description': e[3], 'folder': False
                }
            )
...
class TextBox(pyxbmct.AddonDialogWindow):
    ...
    def set_info_controls(self):
        Background = pyxbmct.Image(xbmc.translatePath(
            os.path.join('special://home/addons/script.adultflix.artwork', 'resources/art/dialog/bg.jpg')))
...
def hard_reset():
    ...
    choice = xbmcgui.Dialog().yesno("[COLOR orangered][B]RESET AdultFlix?[/B][/COLOR]", '[COLOR white]ARE YOU SURE YOU WANT TO RETURN AdultFlix TO THE DEFAULT STATE AND LOSE ALL YOUR INFORMATION?[/COLOR]')
    if choice:
        try:
            ...
            quit()
        kodi.dialog.ok(kodi.get_name(), "[COLOR white]AdultFlix has been reset to the factory state.[/COLOR]", "[COLOR white]Press OK to continue.[/COLOR]")
