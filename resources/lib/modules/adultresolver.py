...
from packlib import client, workers, jsunpack, kodi, log_utils
...
class streamer:
    ...
    def resolve(self, url, pattern=None):
        ...
        if pattern:
            ...
            if 'eporner.com' in url:
            ...
            elif 'girlfriendvideos.com' in url:
                u = self.girlfriendvideos(url)

            elif 'watchxxxfree.com' in url:
                u = self.watchxxxfree(url)

            elif 'porn00' in url:
                u = self.porn00(url)

            elif 'justporno.tv' in url:
                u = self.justporno(url)

            elif '4tube.com' in url:
                u = self.fourtube(url)

            elif 'perfectgirls.net' in url:
                u = self.perfectgirls(url)

            elif 'pornhub.com' in url:
                u = self.pornhub(url)

            elif 'pornheel.com' in url:
                u = self.pornheel(url)

            elif 'pandamovie.eu' in url:
                u = self.pandamovie(url)

            elif 'winporn.com' in url:
                u = self.winporn(url)

            elif 'yuvutu.com' in url:
                u = self.yuvutu(url)

            elif 'huge6.com' in url:
                u = self.hugesix(url)

            elif 'boobsandtits.co.uk' in url:
                u = self.boobntit(url)

            elif 'sexmax.co' in url:
                u = self.sexmax(url)
            ...
            elif 'solopornoitaliani' in url:
                u = self.solopornoitaliani(url)

            elif 'spreadporn.org' in url:
                u = self.spreadporn(url)

            elif 'befuck.com' in url:
                u = self.befuck(url)

            elif 'megasesso' in url:
                u = self.megasesso(url)

            elif 'freeones' in url:
                u = self.freeones(url)

            elif 'fuqer.com' in url:
                u = self.fuqer(url)

            elif 'siska' in url:
                u = self.siska(url)

            elif 'satan18av' in url:
                u = self.satan18av(url)

            elif 'overthumbs' in url:
                u = self.overthumbs(url)

            elif 'streamate.com' in url:
                u = self.streamate(url)

            elif 'mixhdporn.com' in url:
                u = self.mixhd(url)
                ...
                u = self.chaturbate(url)

            elif 'nxgx.com' in url:
                u = self.nxgx(url)

            else:
                u = self.generic(url, pattern=None)
        ...
        try:
            ...
            if pattern:
                ...
                for pattern in patterns:
                    ...
                    s += [i for i in l if urlparse(i).path.strip('/').split('/')[-1].split('.')[-1] in ['mp4', 'flv', 'm3u8']]
            ...
            log_utils.log('Returning %s from AdultFlix Resolver' % str(u), xbmc.LOGNOTICE)
            return u
        except Exception as e:
            log_utils.log('Error resolving %s :: Error: %s' % (url, str(e)), xbmc.LOGERROR)
    ...
    def fourtube(self, url):
        try:
            ...
            r = client.request(urljoin(self.fourtube_base, self.fourtube_embed % id))
            ...
            qual = qual.replace(',', '+')
            r = client.request(self.fourtube_post % (url_id, qual), post='', headers={'Origin': self.fourtube_base},
